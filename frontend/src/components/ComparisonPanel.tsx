...
export function ComparisonPanel({ study, runs }: {
  ...
  // Discover the result tables shared across all selected runs.
  useEffect(() => {
    let cancelled = false
    ...
    Promise.all(unique.map(r => discoverResultsTables(study, r.run, r.group)))
      .then(results => {
        if (cancelled) return
        // Intersect: keep only tables present in ALL runs
        ...
        const sorted = [...shared.values()].sort((a, b) => a.label.localeCompare(b.label))
        setOptions(sorted)
  ...
  const isSubgroupMode = effectiveRuns.length > 0 && effectiveRuns.every(r => r.prefix)
  ...
  return (
    <div style={{ marginTop: 24 }}>
      ...
      <p style={{ fontSize: '.82rem', color: 'var(--color-muted-fg)', marginBottom: 12 }}>
        Comparing {effectiveRuns.length} {isSubgroupMode ? 'groups' : 'runs'} on the{' '}
        {options.length > 1 ? (
          <select
            value={analysisKey ?? ''}
      ...
      {alphaFig != null && (
        <>
          ...
          {ALPHA_METRICS.filter(m => metrics.has(m)).map(m => (
            <ChartCustomiser key={m} study={study} chartType={`alpha_${m}`}
              figure={extractAlphaPanel(alphaFig, m)} heightRatio={0.32} />
          ))}
        </>
      )}
      {nmdsFig != null && (
        <ChartCustomiser study={study} chartType="nmds" figure={nmdsFig} heightRatio={0.48} />
      )}
      {permanova && (
      ...
      {/* Unified taxa / composition chart */}
