...
@testset "write_category_columns! on a merged table" begin
    ...
    library = Dict{String,Any}(
        ...
        "sets" => Dict{String,Any}(
            "default" => Dict("categories" => [
                Dict("name" => "Bacteria", "filter" => "bacteria"),
                Dict("name" => "Other"),  # filter-less catch-all
            ]),
        ),
    )
    Categories.write_category_columns!(con, "merged", "VSEARCH", ["default"]; library)
...
@testset "ensure_columns! is idempotent" begin
    ...
    Categories.ensure_columns!(con, "merged", "VSEARCH", ["default"]; library)
    @test "Category__default" in names(DataFrame(DBInterface.execute(con,
        "SELECT * FROM merged LIMIT 0")))
...
@testset "category exclusion drops the classified rows (mechanism)" begin
    ...
    filters = Dict{String,Any}("vertebrates" => Dict("filters" =>
        [Dict("column" => "Class", "pattern" => "Craniata", "action" => "keep")]))
    cats = [Dict("name" => "Contaminant", "filter" => "vertebrates")]
...
@testset "shipped contamination set keeps only the whitelist" begin
    ...
    wl = filters["eukaryome_whitelist"]
